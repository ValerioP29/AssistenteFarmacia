...
/**
 * Costruisce le espressioni SQL per sconto e prezzo scontato.
 *
 * @param string $priceExpr  Espressione SQL del prezzo base
 * @param string $tableAlias Alias della tabella nella query corrente
 *                           ('pp' nella inner query, 'base' o 'ranked' nella outer)
 */
function build_related_discount_expr($priceExpr, $tableAlias = 'pp'){
	$percentageDiscountExpr = "CASE
		WHEN {$tableAlias}.percentage_discount IS NULL THEN NULL
		WHEN {$tableAlias}.percentage_discount > 0 AND {$tableAlias}.percentage_discount <= 100
			THEN {$tableAlias}.percentage_discount
		ELSE NULL
	END";

	$discountedPriceExpr = "CASE
		WHEN ({$priceExpr}) IS NULL OR ({$priceExpr}) <= 0 THEN NULL
		WHEN {$percentageDiscountExpr} IS NOT NULL
			THEN ROUND(({$priceExpr}) * (1 - ({$percentageDiscountExpr} / 100)), 2)
		ELSE NULL
	END";

	$hasDiscountExpr = "CASE
		WHEN {$tableAlias}.is_on_sale = 1 THEN 1
		WHEN ({$discountedPriceExpr}) IS NOT NULL
			AND ({$discountedPriceExpr}) < ({$priceExpr}) THEN 1
		WHEN {$tableAlias}.discount_type IS NOT NULL
			AND TRIM(CONVERT({$tableAlias}.discount_type USING utf8mb4)
				COLLATE utf8mb4_unicode_ci) <> ''
			AND {$tableAlias}.percentage_discount IS NOT NULL
			AND {$tableAlias}.percentage_discount > 0 THEN 1
		ELSE 0
	END";
...
/**
 * Costruisce la configurazione ORDER BY per i prodotti correlati.
 *
 * @param string      $nameExpr   Espressione SQL del nome
 * @param string      $imageExpr  Espressione SQL dell'immagine
 * @param string      $priceExpr  Espressione SQL del prezzo
 * @param string|null $scoreExpr  Espressione SQL dello score di rilevanza
 * @param string      $tableAlias Alias della tabella nella outer query
...
function build_related_order_by($nameExpr, $imageExpr, $priceExpr, $scoreExpr = null, $tableAlias = 'pp'){
	...
	$hasPriceExpr = "CASE
		WHEN {$priceExpr} IS NOT NULL AND {$priceExpr} > 0 THEN 1 ELSE 0 END";

	$isSentenceCaseExpr = "CASE
		WHEN {$nameExpr} IS NULL OR TRIM({$nameTextExpr}) = '' THEN 0
		WHEN BINARY {$nameExpr} = BINARY CONCAT(
			UPPER(LEFT({$nameExpr}, 1)), LOWER(SUBSTRING({$nameExpr}, 2))) THEN 1
		ELSE 0
	END";
...
function build_safe_column_expr($columnName, $pharmaColumnsMap, $defaultExpr = 'NULL'){
	return !empty($pharmaColumnsMap[$columnName]) ? "pp.{$columnName}" : $defaultExpr;
}
...
function build_product_base_select_sql($global_table_exists, $pharma_columns_map, $includeSearchFilter = false, $exclude_ids = []){
	$baseColumns = [
		...
		build_safe_column_select_expr('tags',                $pharma_columns_map, 'NULL'),
	];
...
if( $pharma_id <= 0 || (!$related_mode && empty($search_term)) ){
	echo json_encode([
		...
		'message' => $related_mode
			...
			: 'Parametri mancanti o non validi. Richiesti: search e pharma_id.',
	]);
	exit();
}
...
try {
	...
	$stmtColumns->execute();
	$pharma_columns_map = [];
	foreach ($stmtColumns->fetchAll(PDO::FETCH_COLUMN) as $col) {
		$pharma_columns_map[(string)$col] = true;
	}
	$tags_column_exists = !empty($pharma_columns_map['tags']);

	if ($debug_enabled) {
		$schema_debug['columns'] = array_keys($pharma_columns_map);
		$schema_debug['global_table_exists'] = $global_table_exists;
		error_log('[product-search][debug] ' . json_encode($schema_debug, JSON_UNESCAPED_UNICODE));
	}
	...
	// ── RELATED MODE ──────────────────────────────────────────────────────────
	if ($related_mode) {
		$select_base = build_product_base_select_sql(
			$global_table_exists, $pharma_columns_map, false, $exclude_ids
		);
		...
		if ($branch === 'generic' && !empty($related_seed_normalized)) {
			...
			$branch = 'seed';
			$whereConditions[] = "(LOWER({$nameSearchExpr}) LIKE LOWER(:seed_like)
	...
	$search_select_base = build_product_base_select_sql(
		$global_table_exists, $pharma_columns_map, true, $exclude_ids
	);

	$sql = "SELECT base.*,
		CASE
			WHEN LOWER(CONVERT(base.name USING utf8mb4) COLLATE utf8mb4_unicode_ci)
				LIKE LOWER(:search_prefix) THEN 3
			WHEN LOWER(CONVERT(base.name USING utf8mb4) COLLATE utf8mb4_unicode_ci)
				LIKE LOWER(:search_term) THEN 2
			WHEN LOWER(CONVERT(base.description USING utf8mb4) COLLATE utf8mb4_unicode_ci)
				LIKE LOWER(:search_term) THEN 1
			ELSE 0 END AS search_relevance
		FROM ({$search_select_base}) base
		ORDER BY
			CASE WHEN base.image IS NOT NULL
				AND TRIM(CONVERT(base.image USING utf8mb4) COLLATE utf8mb4_unicode_ci) <> ''
				AND LOWER(TRIM(CONVERT(base.image USING utf8mb4) COLLATE utf8mb4_unicode_ci))
					NOT LIKE '%placeholder%'
				THEN 1 ELSE 0 END DESC,
			CASE WHEN base.name IS NOT NULL
				AND TRIM(CONVERT(base.name USING utf8mb4) COLLATE utf8mb4_unicode_ci) <> ''
				AND BINARY base.name = BINARY LOWER(base.name)
				THEN 1 ELSE 0 END DESC,
			CASE WHEN base.name IS NOT NULL
				AND TRIM(CONVERT(base.name USING utf8mb4) COLLATE utf8mb4_unicode_ci) <> ''
				AND BINARY base.name = BINARY CONCAT(
					UPPER(LEFT(base.name, 1)), LOWER(SUBSTRING(base.name, 2)))
				THEN 1 ELSE 0 END DESC,
	...
	error_log('[product-search] SQL error: ' . $e->getMessage()
		. ' | SQLSTATE: ' . $sqlState
		. ' | context=' . json_encode($errorContext, JSON_UNESCAPED_UNICODE));
